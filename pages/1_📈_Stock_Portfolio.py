...
def fetch_data():
    try:
        # 1. CONNECT USING SHARED MODULE
        wb = connect_to_sheet(SHEET_NAME) # Returns the Workbook object
        
        # 2. Portfolio Data
        sheet = wb.worksheet("Share Portfolio")
        data = sheet.get_all_values()
        df = pd.DataFrame(data[1:], columns=[str(h).strip() for h in data[0]])
        
        # 3. History Data
        try:
            hist_sheet = wb.worksheet(HISTORY_TAB_NAME)
        ...
        try:
            ...
            # Chart Data
            try:
                c_sheet = m_wb.worksheet(CHART_TAB_NAME)
...
def update_history_log(current_val):
    try:
        ...
        wb = connect_to_sheet(SHEET_NAME)
        h_sheet = wb.worksheet(HISTORY_TAB_NAME)
    ...
    for idx, row in portfolio.iterrows():
        ...
        if "PRIVATE" in t:
            ...
            try:
                ...
                # We find the matching row in raw_sheet_data where Column A matches Ticker
            ...
            # If manual price is valid, use it. Else, fallback to Purchase Price.
